...
Page({
    ...
    chatInfo.doc(app.globalData._id).get({
      success(res) {
        ...
        wx.setNavigationBarTitle({
          title: that.data.chatInfo.sellerName.toString()
        })
      }
    })
    myChat.doc(app.globalData._id).get({
      success(res) {
        if (res.data.chats.length > that.data.chatInfo.chats.length) {
          chatInfo.doc(app.globalData._id).update({
            data: {
  ...
  formSubmit(e) {
    ...
    if (e.detail.value.textarea != "") {
      ...
      chatInfo.doc(that.data.chatInfo._id).update({
        data: {
          chats: _.push(chat)
        },
        success() {
          chatInfo.doc(that.data.chatInfo._id).get({
            success(res) {
              that.setData({
                chatInfo: res.data
              })
